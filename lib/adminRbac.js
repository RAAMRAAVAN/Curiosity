...
export const ensureTeachersRoleExists = async () => {
  ...
  if (existing) {
    const permissions = uniqueStrings([
      ...
      ...DEFAULT_TEACHERS_ROLE_PERMISSIONS,
    ]);
    if (permissions.length !== (existing.permissions || []).length) {
      const updated = { ...existing, permissions, updatedAt: new Date().toISOString() };
      const roles = await getAllCustomRoles();
      await saveAllCustomRoles(roles.map((item) => item.id === existing.id ? updated : item));
      return updated;
    }
    return existing;
  }
